...
upstreams = [
    attr for attr in dir(hubconf) if callable(getattr(hubconf, attr)) and attr[0] != "_"
]
parser.add_argument("--mode", choices=["list", "help", "load"], required=True)
parser.add_argument("--upstream", choices=upstreams)
parser.add_argument(
    "--ckpt",
    help="The PATH/URL/GOOGLE_DRIVE_ID of upstream checkpoint, not always needed",
)
parser.add_argument("--config", help="The PATH of upstream config, not always needed")
parser.add_argument(
    "--refresh", action="store_true", help="Whether to re-download upstream contents"
)
