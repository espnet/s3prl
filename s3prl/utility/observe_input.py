...
def main():
    ...
    plot_x(
        x_time_zero.data.cpu().numpy(),
        name="x_time_zero",
        xlabel="B) Mask contiguous segments to zero along temporal axis",
    )
    ...
    plot_x(
        x_time_replace.data.cpu().numpy(),
        name="x_time_replace",
        xlabel="C) Replace contiguous segments with random segments",
    )
    ...
    x_all[:, chosen_intervals] = 0
    plot_x(
        x_freq.data.cpu().numpy(),
        name="x_freq",
        xlabel="D) Mask contiguous segments to zero along channel axis",
    )
    ...
    plot_x(
        x_noise.data.cpu().numpy(),
        name="x_noise",
        xlabel="E) Apply sampled Gaussian noise to magnitude",
    )

    # time + freq + noise
    plot_x(
        x_all.data.cpu().numpy(),
        name="x_all",
        xlabel="F) Combining the alterations in B), D), and E)",
    )
