...
from s3prl.utility.helper import defaultdict, hack_isinstance, override
...
def get_ttest_args():
    ...
    parser.add_argument(
        "-m", "--mode", choices=["ttest", "fisher", "mcnemar"], default="ttest"
    )
...
def process_records(records, metric):
    assert (
        "sample_wise_metric" in records
    ), "Utterance-wise / sample-wise metric is necessary for proceeding the Paired Sample T-test."
