...
def run_over_layer(layer, real_todo, adve_todo):
    ...
    for i in range(len(real_todo)):
        ...
        if layer == "feature":
            ...
            l2 = compute_lnsr(
                r_hidd.data.cpu().numpy(), a_hidd.data.cpu().numpy(), norm_L2=True
            )
        episode.append(l2)
...
def main():
    num_layers = 12
    data_path = (
        "data/adversarial/"  # this can be downloaded from the S3PRL google drive
    )
    ...
    print(
        "[Original v.s. "
        + data_type
        + "] Hidden rep distance over all layers (1->12): ",
        dist,
    )
