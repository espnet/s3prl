...
def sent_normalise(text, slots_split=None):
    ...
    for idx in range(len(text_split)):
        ...
        if text_split[idx] in reservations:
            ...
            continue
        norm_text = normalise(
            word_normalise([text_split[idx]]), variety="AmE", verbose=False
        )
...
def process_daniel_snips_file(content):
    ...
    for line in content:
        ...
        cleaned_slots = " ".join(
            [x.split("/")[1] if x != "O" else x for x in cleaned_slots.split()]
        )
    ...
    assert (
        len(set(valid_utt2intent.values()))
        == len(set(test_utt2intent.values()))
        == len(set(train_utt2intent.values()))
        == 7
    )
    ...
    assert (
        len(__return_set_of_slots(valid_utt2slots))
        == len(__return_set_of_slots(test_utt2slots))
        == len(__return_set_of_slots(train_utt2slots))
        == 40
    )
...
def map_and_link_snips_audio(snips_audio_dir, link_dir):
    ...
    result = [
        y for x in os.walk(snips_audio_dir) for y in glob(os.path.join(x[0], "*.mp3"))
    ]
...
def create_multispk_for_snips(output_dir):
    ...
    dataset_info = [
        {"split": "test", "num_utts": 700},
        {"split": "valid", "num_utts": 700},
        {"split": "train", "num_utts": 13084},
    ]
    ...
    for data in dataset_info:
        ...
        split = data["split"]
        with open(
            os.path.join(output_dir, "single-matched-snips.%s.w-intent" % split)
        ) as f:
            content = f.readlines()
...
def apply_text_norm_and_modify_slots(all_tsv, output_dir):
    ...
    # test
    test_file = open(
        os.path.join(output_dir, "single-matched-snips.test.w-intent"), "w"
    )
    vocab_slot = {}
    ...
    # valid
    valid_file = open(
        os.path.join(output_dir, "single-matched-snips.valid.w-intent"), "w"
    )
    for uttid in tqdm.tqdm(
        valid_dirs[0].keys(), desc="Text Normalising on validation set"
    ):
    ...
    # train
    train_file = open(
        os.path.join(output_dir, "single-matched-snips.train.w-intent"), "w"
    )
    for uttid in tqdm.tqdm(
        train_dirs[0].keys(), desc="Text Normalising on training set"
    ):
    ...
    vocab_file.write(
        "\n".join(
            sorted(list(vocab_slot.keys()), key=lambda x: vocab_slot[x], reverse=True)
        )
    )
    ...
    for name in tqdm.tqdm(files, desc="Process for speaker: " + speaker):
        if name.endswith(".mp3"):
            ...
            new_file = os.path.join(
                out_dir, speaker + "-" + name.split("/")[-1].split(".")[0] + ".wav"
            )
            bashCommand = (
                "sox "
                + orig_file
                + " -t wav -c 1 -r 16000 -b 16 -e signed-integer "
                + new_file
            )
...
def sox_mp3_to_wav(in_root, out_root):
    ...
    for root, dirs, files in os.walk(in_root):
        ...
        print(
            "[Processing] process %d audio files from speaker %s"
            % (len(files), speaker)
        )
...
if __name__ == "__main__":
    ...
    if mode == "text":
        ...
        content = []
        content += open(
            os.path.join(repo_dir, "data/nlu_annotation/valid")
        ).readlines()[1:]
        content += open(os.path.join(repo_dir, "data/nlu_annotation/test")).readlines()[
            1:
        ]
        content += open(
            os.path.join(repo_dir, "data/nlu_annotation/train")
        ).readlines()[1:]
