...
def get_preprocess_args():
    ...
    parser.add_argument(
        "--data_path",
        default="/home/leo/d/datasets/MOSI/Raw/Audio/WAV_16000/Segmented",
        type=str,
        help="Path to raw MOSI segmented audio dataset",
    )
    parser.add_argument(
        "--output_path",
        default="../data/",
...
def acoustic_preprocess(args, dim):
    ...
    output_dir = os.path.join(
        args.output_path, "_".join(["mosi", str(args.feature_type) + str(dim)])
    )
    ...
    tr_x = Parallel(n_jobs=args.n_jobs)(
        delayed(extract_feature)(
            ...
            cmvn=args.apply_cmvn,
            save_feature=os.path.join(
                output_dir, str(file).split("/")[-1].replace(".wav", "")
            ),
        )
    ...
    def classify(file_name):
        ...
        shutil.move(
            os.path.join(output_dir, file_name), os.path.join(npy_dir, file_name)
        )
        if prefix in train_split:
            ...
            assert 0, "Error in preprocess_mosi.py:146"

    belong = np.apply_along_axis(
        lambda file_name: classify(file_name), 1, df["file_path"].values.reshape(-1, 1)
    )
