...
from utility.audio import extract_feature, num_freq, num_mels, num_mfcc
...
def preprocess(args, dim):
    for s in ("train", "dev", "test"):
        ...
        todo = list(
            Path(os.path.join(args.data_path, s.upper())).rglob("*.[wW][aA][vV]")
        )
        if len(todo) == 0:
            todo = list(Path(os.path.join(args.data_path, s)).rglob("*.[wW][aA][vV]"))
        print(len(todo), f"audio files found in {s} set")

        if args.name == "None":
            output_dir = os.path.join(
                args.output_path, "_".join(["timit", str(args.feature_type) + str(dim)])
            )
        else:
        ...
        tr_x = Parallel(n_jobs=args.n_jobs)(
            delayed(extract_feature)(
                ...
                save_feature=os.path.join(
                    cur_path, str(file).split("/")[-1].split(".")[0]
                ),
            )
        ...
        # sort by len
        sorted_todo = [
            os.path.join(s, str(todo[idx]).split("/")[-1].split(".")[0] + ".npy")
...
def main():
    ...
    dim = (
        ...
        else (mfcc_dim if args.feature_type == "mfcc" else mel_dim)
    )
    print(
        "Delta: ",
        args.delta,
