...
LIBRI_PATH = os.path.join(
    KALDI_ROOT, "egs/librispeech/s5/" + DATA_TYPE + "_cmvn/"
)  # this needs to be generated by the scripts `dump_fmllr_cmvn.sh` or `dump_mfcc_cmvn.sh`
...
def main():
    ...
    if not os.path.isdir(LIBRI_PATH):
        ...
        print(
            "Please run the kaldi scripts first! More information are described in the README file and Wiki page."
        )
    ...
    for s in SETS:
        with ReadHelper(
            "ark:" + LIBRI_PATH + s + "/" + DATA_TYPE + "_cmvn.ark"
        ) as reader:
            ...
            df = pd.DataFrame(
                data={
                    "file_path": [fp for fp, l in output],
                    "length": [l for fp, l in output],
                    "label": "None",
                }
            )
