...
def acoustic_preprocess(args, tr_set, dim, audio_extention):
    ...
    for i, s in enumerate(tr_set):
        ...
        todo = list(
            Path(os.path.join(args.data_root, s)).rglob("*" + audio_extention)
        )  # '*.flac'
        ...
        if args.name == "None":
            output_dir = os.path.join(
                args.output_path,
                "_".join(["NewData", str(args.feature_type) + str(dim)]),
            )
        else:
        ...
        tr_x = Parallel(n_jobs=args.n_jobs)(
            delayed(extract_feature)(
                ...
                save_feature=os.path.join(
                    cur_path, str(file).split("/")[-1].replace(audio_extention, "")
                ),
            )
        ...
        sorted_todo = [
            os.path.join(
                tr_set[i],
                str(todo[idx]).split("/")[-1].replace(audio_extention, ".npy"),
            )
