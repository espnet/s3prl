...
def get_preprocess_args():
    ...
    parser.add_argument(
        "-i",
        "--input_path",
        default="/livingrooms/public/LibriLight/",
        type=str,
    ...
    parser.add_argument(
        ...
        default="-splitted",
        type=str,
        help="Name to append on the original directory",
        required=False,
    )
    parser.add_argument(
        "--n_jobs",
        default=-1,
        type=int,
        help="Number of jobs used for computation",
        required=False,
    )
...
def generate_splits(args, tr_set, audio_extension):
    ...
    for i, s in enumerate(tr_set):
        ...
        print("")
        todo = list(
            Path(os.path.join(args.input_path, s)).rglob("*" + audio_extension)
        )  # '*.flac'
        print(f"Preprocessing data in: {s}, {len(todo)} audio files found.")

        print("Splitting audio to shorter length...", flush=True)
        Parallel(n_jobs=args.n_jobs)(
            delayed(split_and_save)(str(file), s, args) for file in tqdm(todo)
        )
