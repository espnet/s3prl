...
def get_preprocess_args():
    ...
    parser.add_argument(
        "--data_path",
        default="/home/leo/d/datasets/MOSEI/Raw/Audio/Full/WAV_16000",
        type=str,
        help="Path to MOSEI non-segmented WAV files",
    )
    parser.add_argument(
        "--output_path",
        default="../../data/mosei",
        type=str,
        help="Path to store segmented flac and npys. Should already contains mosei_no_semi.csv",
        required=False,
    )
...
def segment_mosei(args):
    ...
    assert os.path.exists(
        mosei_summary
    ), "Output path should already be created with a mosei_no_semi.csv inside it"
    ...
    for index, row in df.iterrows():
        ...
        seg_wav.export(
            seg_flacpath,
            format="flac",
            parameters=["-ac", "1", "-sample_fmt", "s16", "-ar", "16000"],
        )
