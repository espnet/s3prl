...
def generate_masked_acoustic_model_data(spec, config):
    ...
    with torch.no_grad():
        ...
        if (
            len(spec) == 2
        ):  # if self.duo_feature: dataloader will output `source_spec` and `target_spec`
            spec_masked = spec[0]
            spec_target = spec[1]
        elif len(spec) == 1:
        ...
        pos_enc = fast_position_encoding(
            seq_len, config["position_encoding_size"]
        )  # (seq_len, position_encoding_size)
        mask_label = (
            torch.zeros_like(spec_target, dtype=torch.uint8)
        ...
        for idx in range(batch_size):
            ...
            if config["mask_proportion"] > 0:
                mask_consecutive = random.randint(
                    config["mask_consecutive_min"], config["mask_consecutive_max"]
                )
                valid_start_max = max(
                    spec_len[idx] - mask_consecutive - 1, 0
                )  # compute max valid start point for a consecutive mask
                proportion = round(
                    spec_len[idx] * config["mask_proportion"] / mask_consecutive
                )
                if config["mask_allow_overlap"]:
                    ...
                    mask_bucket_size = round(
                        mask_consecutive * config["mask_bucket_ratio"]
                    )
                ...
                # replace to random frames
                    ...
                    spec_masked[idx, chosen_intervals, :] = spec_masked[
                        idx, random_intervals, :
                    ]
                # do nothing
                else:
                    pass
            ...
            if config["mask_frequency"] > 0:
                ...
                chosen_starts = torch.randperm(spec_masked.shape[2] - rand_bandwidth)[
                    :1
                ]
        ...
        if config["noise_proportion"] > 0:
            ...
            if dice < config["noise_proportion"]:
                ...
                spec_masked += noise_sampler.sample(spec_masked.shape).to(
                    device=spec_masked.device
                )

        valid_batchid = (
            mask_label.view(batch_size, -1).sum(dim=-1).nonzero(as_tuple=False).view(-1)
        )
