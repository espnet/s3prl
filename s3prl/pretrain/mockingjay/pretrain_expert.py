...
class UpstreamPretrainExpert(nn.Module):
    ...
    def __init__(
        ...
        if "libri_root" in self.datarc and "kaldi" in self.upstream_config["audio"]:
            print(
                "[UpstreamPretrainExpert] - Using kaldi feature extracter, on-the-fly feature extraction"
            )
            ...
            extracter, input_dim, output_dim = get_preprocessor(
                self.upstream_config["audio"]
            )
        else:
            print("[UpstreamPretrainExpert] - Using features pre-extracted and saved")
            extracter, input_dim = (
                None,
                self.upstream_config["transformer"]["input_dim"],
            )
            output_dim = None
        ...
        self.model = TransformerForMaskedAcousticModel(
            model_config, input_dim, output_dim=output_dim
        )
    ...
    def loss_to_device(self):
        (
            ...
            else self.model.module.loss.to(self.device)
        )

    # Interface
    def add_state_to_save(self, all_states):
        all_states["SpecHead"] = (
            self.model.SpecHead.state_dict()
            if not self.multi_gpu
            else self.model.module.SpecHead.state_dict()
        )
        all_states["Transformer"] = (
            self.model.Transformer.state_dict()
            if not self.multi_gpu
    ...
    def forward(self, data, records={}, global_step=0, log_step=1000, **kwargs):
        ...
        if pos_enc.dim() == 3:
            ...
            pos_enc = pos_enc.to(self.device).expand(
...
class TransformerForMaskedAcousticModel(TransformerInitModel):
    ...
    def __init__(
        ...
        loss = {"L1": nn.L1Loss(), "MSE": nn.MSELoss()}
    ...
    def forward(
        ...
        if spec_label is not None and mask_label is not None:
            assert (
                mask_label.sum() > 0
            ), "Without any masking, loss might go NaN. Check your pretrain data processing (s3prl/pretrain/mockingjay/task.py)"
