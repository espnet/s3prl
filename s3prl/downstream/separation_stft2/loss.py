...
class SepLoss(object):
    ...
    def compute_loss(self, masks, feat_length, source_attr, target_attr):
        ...
        targets_spect = torch.stack(
            [targets_spect[:, perm, :, :] for perm in perm_list], dim=0
        )
        targets_phase = torch.stack(
            [targets_phase[:, perm, :, :] for perm in perm_list], dim=0
        )
        if self.mask_type == "AM":
            refer_spect = targets_spect
        elif self.mask_type == "PSM":
            refer_spect = targets_spect * torch.cos(
                mixture_phase.unsqueeze(0).unsqueeze(2) - targets_phase
            )
        elif self.mask_type == "NPSM":
            refer_spect = targets_spect * F.relu(
                torch.cos(mixture_phase.unsqueeze(0).unsqueeze(2) - targets_phase)
            )
        else:
