...
parser.add_argument(
    "--sample_rate", type=str, choices=["8k", "16k"], default="16k", help="Sample rate"
)
parser.add_argument(
    "--mode", type=str, choices=["max", "min"], default="min", help="Length mode"
)
parser.add_argument(
    "--part",
    type=str,
    ...
    default="dev",
    help="Partition of dataset",
)
args = parser.parse_args()


def main():
    output_dir = "{}/wav{}/{}/{}".format(
        args.tgt_dir, args.sample_rate, args.mode, args.part
    )
    ...
    wav_dir = "{}/wav{}/{}/{}".format(
        args.src_dir, args.sample_rate, args.mode, args.part
    )
    ...
    for cond in ["s1", "s2", "mix_clean", "mix_both", "mix_single", "noise"]:
        ...
        filelist = [
            f for f in os.listdir("{}/{}".format(wav_dir, cond)) if f.endswith(".wav")
        ]
        filelist.sort()
