...
class SpeakerDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        assert os.path.isdir(
            bucket_file
        ), "Please first run `preprocess/generate_len_for_bucket.py to get bucket file."
        self.table = pd.read_csv(
            os.path.join(bucket_file, "train-clean-100.csv")
        ).sort_values(by=["length"], ascending=False)
        X = self.table["file_path"].tolist()
        X_lens = self.table["length"].tolist()

        if (split == "train" or split == "dev") and os.path.isfile(
            os.path.join(split_file, "train_split.txt")
        ):
            ...
            usage_list = (
                usage_list[:percent] if split == "train" else usage_list[percent:]
            )
        elif split == "test" and os.path.isfile(
            os.path.join(split_file, "test_split.txt")
        ):
        ...
        if len(batch_x) > 1:
            if (
                self._parse_x_name(x) in usage_list
            ):  # check if x is in list if list not empty
                self.X.append(batch_x)
        ...
        print(
            "[Dataset] - # possible speaker classes: "
            + str(self.class_num)
            + ", number of data for "
            + split
            + ": "
    ...
    def _compute_speaker2idx(self, speakers):
        ...
        for speaker in sorted(speakers):
            if (
                speaker not in speaker2idx and speakers[speaker] > SPEAKER_THRESHOLD
            ):  # eliminate the speakers with too few utterance
    ...
    def __getitem__(self, index):
        ...
        label_batch = torch.LongTensor(
            [
                self.speaker2idx[self._get_speaker_from_path(x_file)]
