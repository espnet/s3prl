...
from .dataset import SpeakerVerifi_test, SpeakerVerifi_train
from .model import AAMSoftmaxLoss, AMSoftmaxLoss, Model, SoftmaxLoss, UtteranceExtractor
...
class DownstreamExpert(nn.Module):
    ...
    def log_records(self, mode, records, logger, global_step, **kwargs):
        ...
        if mode == "train":
            ...
            logger.add_scalar(
                f"sv-voxceleb1/{mode}-loss", loss, global_step=global_step
            )
            ...
            def names(name):
                return "-".join(name.split("/")[-3:]).split(".")[0]  # added
