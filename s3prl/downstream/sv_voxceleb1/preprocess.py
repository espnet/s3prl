...
if __name__ == "__main__":
    ...
    construct_dev_speaker_id_txt(
        dev_speakers, "./downstream/sv_voxceleb1/dev_meta_data/dev_meta_speaker_ids.txt"
    )
    wav_list = sample_wavs_and_dump_txt(
        args.root,
        dev_speakers,
        args.sample_pair,
        "./downstream/sv_voxceleb1/dev_meta_data/dev_meta_data_voxceleb2.txt",
    )
    ...
    print(
        f"copy speaker_folder from {args.root}/dev/wav to {args.output_dir}/dev/wav .."
    )
    for speaker_fold in tqdm.tqdm(dev_speakers):
        shutil.copytree(
            root / speaker_fold,
            new_dir / "dev" / "wav" / speaker_fold,
            dirs_exist_ok=True,
        )
