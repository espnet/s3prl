try:
    from cmu_mosei_std_folds import *

    standard_train_fold = set(standard_train_fold)
    standard_valid_fold = set(standard_valid_fold)
    standard_test_fold = set(standard_test_fold)
except ImportError:
    standard_train_fold = set(
        [
            "hh04W3xXa5s",
            "GdFP_p4eQX0",
            "4iG0ffmnCOw",
...
Raw_Label_Path = "./CMU_MOSEI_Labels.csd"

files = []  # ID for Each Video
indexs = []  # Index of Segments in the Same Video
...
def label2a(a):  # negative/non-negative: simply <0 or >=0 (used in most papers)
    if a < 0:
        return 0
    return 1
...
for file in data.computational_sequences["All Labels"].keys():
    for index in range(
        len(data.computational_sequences["All Labels"][file]["intervals"])
    ):
        files.append(file)
        indexs.append(index)
        starts.append(
            data.computational_sequences["All Labels"][file]["intervals"][index][0]
        )
        ends.append(
            data.computational_sequences["All Labels"][file]["intervals"][index][1]
        )
        ...
        labels_2a.append(
            label2a(
                data.computational_sequences["All Labels"][file]["features"][index][0]
            )
        )
        labels_2b.append(
            label2b(
        ...
        labels_6.append(
            data.computational_sequences["All Labels"][file]["features"][index][
                1:
            ].argmax()
        )
        labels_7.append(
        ...
        if file in standard_train_fold:
            splits.append(0)
        elif file in standard_valid_fold:
            splits.append(1)
        elif file in standard_test_fold:
            splits.append(2)
        else:
            splits.append(3)
...
df = pd.DataFrame(
    data=np.stack(
        [files, indexs, starts, ends, labels_2a, labels_2b, labels_6, labels_7, splits],
        axis=1,
    ),
    columns=[
        "file",
...
print("Class -2: ", labels_7.count(-2))
print("Class -1: ", labels_7.count(-1))
print("Class +0: ", labels_7.count(+0))
print("Class +1: ", labels_7.count(+1))
print("Class +2: ", labels_7.count(+2))
print("Class +3: ", labels_7.count(+3))
print()
print("Conversion of CMU_MOSEI_Labels.csd to CMU_MOSEI_Labels.csv complete!")
