...
def get_parser():
    ...
    parser.add_argument(
        "--task", type=str, required=True, choices=["task1", "task2"], help="task"
    )
    ...
    parser.add_argument(
        "--expdir", type=str, default="result/downstream", help="expdir"
    )
...
if __name__ == "__main__":
    ...
    epochs = list(
        range(args.start_epoch, args.end_epoch + args.step_epoch, args.step_epoch)
    )
    bests = []
    ...
    for trgspk in trgspks:
        ...
        for ep in epochs:
            ...
            log_file = os.path.join(
                args.expdir,
                f"a2o_vc_vcc2020_{args.tag}_{trgspk}_{args.upstream}",
                str(ep),
                f"{args.vocoder}_wav",
                "obj.log",
            )
            if args.task == "task1":
                result = (
                    grep(log_file, "Mean")[0]
                    .split("Mean MCD, f0RMSE, f0CORR, DDUR, CER, WER, accept rate: ")[1]
                    .split(" ")
                )
            elif args.task == "task2":
                result = (
                    grep(log_file, "Mean")[0]
                    .split("Mean CER, WER, accept rate: ")[1]
                    .split(" ")
                )
    ...
    if args.task == "task1":
        avg = [
            f"{(sum([float(best[i]) for best in bests]) / 4.0):.2f}" for i in range(7)
        ]
    elif args.task == "task2":
        avg = [
            f"{(sum([float(best[i]) for best in bests]) / 6.0):.2f}" for i in range(3)
        ]
