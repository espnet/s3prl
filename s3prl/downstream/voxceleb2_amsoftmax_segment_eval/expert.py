...
from .dataset import SpeakerVerifi_dev, SpeakerVerifi_test, SpeakerVerifi_train

# -------------#
from .model import AdMSoftmaxLoss, Model, UtteranceModel
...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, downstream_expert, **kwargs):
        ...
        self.train_dataset = SpeakerVerifi_train(
            self.datarc["vad_config"], **self.datarc["train"]
        )
        self.dev_dataset = SpeakerVerifi_dev(
            self.datarc["vad_config"],
            self.datarc["segment_config"],
            **self.datarc["dev"],
        )
        self.test_dataset = SpeakerVerifi_test(
            self.datarc["vad_config"],
            self.datarc["segment_config"],
            **self.datarc["test"],
        )
        ...
        self.model = Model(
            ...
            agg_dim=self.modelrc["agg_dim"],
            agg_module=self.modelrc["agg_module"],
            config=self.modelrc,
        )
        self.objective = AdMSoftmaxLoss(
            self.modelrc["input_dim"], self.train_dataset.speaker_num, s=30.0, m=0.4
        )
        ...
        if self.training:
            ...
            if len(labels) > 1:
                ...
                for index in range(len(agg_vec)):
                    ...
                    records[f"pairid_{pair_list[index]}"].append(
                        f"utterid_{utterid_list[index]}"
                    )
    ...
    def log_records(
        ...
        if not self.training:
            ...
            EER_result = self.eval_metric(
                np.array(records["ylabels"]), np.array(records["scores"])
            )

            records["EER"] = EER_result[0]

            logger.add_scalar(
                f"{mode}-" + "EER", records["EER"], global_step=global_step
            )
