...
class DownstreamExpert(nn.Module):
    ...
    def get_dataloader(self, split):
        if split == "train":
            ...
            return self._get_eval_dataloader(
                CustomDataset(
                    self.datarc["eval_pair_list_file"], self.datarc["spk_emb_source"]
                )
            )
        ...
        **kwargs,
        ...
        if split == "custom_test":
            predicted_features, predicted_feature_lengths = self.model(
                input_features, input_feature_lengths, ref_spk_embs
            )
    ...
    def log_records(
        ...
        if split == "custom_test":
            ...
            for i, save_wav_name in enumerate(
                tqdm(
                    list(records["save_wav_names"]),
                    dynamic_ncols=True,
                    desc="Saving files",
                )
            ):
            ...
            if split in ["dev", "test"]:
                ...
                for i, (wav_path, ref_spk_name) in enumerate(
                    tqdm(
                    ...
                    if split == "dev":
                        ...
                        os.makedirs(
                            os.path.join(hdf5_save_dir, num_samples), exist_ok=True
                        )
                        os.makedirs(
                            os.path.join(wav_save_dir, num_samples), exist_ok=True
                        )
                        hdf5_save_path = os.path.join(
                            hdf5_save_dir,
                            num_samples,
                            "_".join([ref_spk_name] + wav_path.split("/")[-2:]).replace(
                                ".wav", ".h5"
                            ),
                        )
                        wav_save_path = os.path.join(
                            wav_save_dir,
                            num_samples,
                            "_".join([ref_spk_name] + wav_path.split("/")[-2:]),
                        )
