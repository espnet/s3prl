...
class PhoneDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.Y = {}
        phone_file = open(
            os.path.join(phone_path, "converted_aligned_phones.txt")
        ).readlines()
        ...
        if split == "train":
            ...
            usage_list = [
                line for line in train_list if line.split("-")[2][:2] in ("SI", "SX")
            ]  # 462 speakers, 3696 sentences, 3.14 hr
            ...
            if split == "dev":
                usage_list = [
                    ...
                    if not line.split("-")[1].lower() in TEST_SPEAKERS
                ]  # held-out speakers from test
            else:
                usage_list = [
                    line
                    for line in usage_list
                    if line.split("-")[1].lower() in TEST_SPEAKERS
                ]  # 24 core test speakers, 192 sentences, 0.16 hr
        else:
        ...
        table = pd.read_csv(
            os.path.join(bucket_file, "TRAIN.csv" if split == "train" else "TEST.csv")
    ...
    def __getitem__(self, index):
        ...
        label_batch = [
            torch.LongTensor(self.Y[self._parse_x_name(x_file).upper()])
