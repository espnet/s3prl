...
class AudioBatchData(Dataset):
    def __init__(
        self,
        file_path,
        max_timestep=16000 * 5,
        meta_data=None,
        utter_number=5,
        sizeWindow=1,
        nProcessLoader=1,
        MAX_SIZE_LOADED=4000,
        batch_size=16,
    ):
        ...
        self.all_speakers = [
            ...
            for f in os.scandir(self.roots[key])
            if f.is_dir() and f.path.split("/")[-1] not in self.black_list_speakers
        ]
    ...
    def prepare(self):
        ...
        print(
            f"Scanned {self.totSize} sequences "
    ...
    def getDataLoader(self, batchSize, numWorkers=0, onLoop=-1):
        r"""
    ...
    def getBaseSampler(self, batchSize, offset):
        ...
        sampler = UniformAudioSampler(
            len(self.data), self.sizeWindow, self.utter_number, batchSize, offset
        )
...
class UniformAudioSampler(Sampler):
    ...
    def __init__(self, dataSize, sizeWindow, utter_number, batch_size, offset):
    ...
    def __iter__(self):
        return iter(
            (self.offset + self.utter_number * torch.randperm(self.len)).tolist()
        )
...
class SpeakerVerifi_dev(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (
            wav1,
            wav2,
            torch.tensor(length1),
            torch.tensor(length2),
