...
def collect_audio_batch(batch, split, half_batch_size_wav_len=300000):
    ...
    audio_len, file, audio_feat, text = zip(
        *[
            ...
            for feat_len, f_name, feat, txt in sorted(
                zip(audio_len, file, audio_feat, text), reverse=True, key=lambda x: x[0]
            )
        ]
    )
...
def load_dataset(split, tokenizer, corpus):
    ...
    dataset, loader_bs = create_dataset(
        split, tokenizer, num_workers=num_workers, **corpus
    )
