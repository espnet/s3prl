...
class SnipsDataset(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.speaker_list = (
            kwargs[f"{split}_speakers"]
            if type(split) == str
            else kwargs[f"{split[0]}_speakers"]
        )

        # Load transcription
        transcripts_file = open(
            join(
                self.path,
                (
                    "all.iob.snips.txt"
                    if "-slot" in tokenizer.token_type
                    else "all-trans.txt"
                ),
            )
        ).readlines()
        ...
        for s in split:
            ...
            for i in trange(len(split_list), desc="checking files"):
                uid = (
                    str(split_list[i]).split("/")[-1].split(".wav", 1)[0].split("/")[-1]
                )
            ...
            split_list = new_list
            print(
                f"loaded audio from {len(self.speaker_list)} speakers {str(self.speaker_list)} with {len(split_list)} examples."
            )
        ...
        text = [
            transcripts[str(f).split(".wav", 1)[0].split("/")[-1]] for f in file_list
        ]
