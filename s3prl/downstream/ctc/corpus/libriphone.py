...
class LibriPhoneDataset(Dataset):
    def __init__(
        self, split, tokenizer, bucket_size, path, lexicon, ascending=False, **kwargs
    ):
        # Setup
        self.path = path
        ...
        for word, phonemes_all in word2phonemes_all.items():
            if len(phonemes_all) > 1:
                print(
                    f"[LibriPhone] - {len(phonemes_all)} of phoneme sequences found for {word}."
                )
                for idx, phonemes in enumerate(phonemes_all):
                    print(f"{idx}. {phonemes}")
            word2phonemes[word] = phonemes_all[0]
        print(
            f"[LibriPhone] - Taking the first phoneme sequences for a deterministic behavior."
        )
