...
input_wav = TRANSFORMER.load_data(
    example_wav, **model.config["online"]
)  # size: (seq_len, dim) = (97200, 1)

# forward
input_wav = input_wav.unsqueeze(
    0
)  # add batch dim, size: (batch, seq_len, dim) = (1, 97200, 1)
output_repr = model(
    input_wav
...
print(
    "output_repr size:", output_repr.size()
)  # size: (batch, seq_len, dim) = (1, 608, 768)
