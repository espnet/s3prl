...
class TRANSFORMER(nn.Module):
    def __init__(self, options, inp_dim, config=None):
        ...
        self.out_dim = (
            self.hidden_size
        )  # 768, This attribute is for pytorch-kaldi and downstream runner
    ...
    def load_model(self, state_dict):
        try:
            ...
            print("[Transformer] - Pre-trained weights NOT loaded!")
    ...
    def down_sample_frames(self, spec):
        ...
        if left_over != 0:
            spec = spec[:, :-left_over, :]
        spec_stacked = spec.view(
            spec.shape[0], spec.shape[1] // self.dr, spec.shape[2] * self.dr
        )
        return spec_stacked
    ...
    def process_input_data(self, spec):
        ...
        # input `spec` should have shape BxTxD
            ...
            raise ValueError(
                "Input argument `spec` has invalid shape: {}".format(spec.shape)
            )

        # Down sample
        if self.dr > 1:
            spec_stacked = self.down_sample_frames(
                spec
        ...
        spec_len = np.sum(
            np.sum(spec_stacked.cpu().data.numpy(), axis=-1) != 0, axis=-1
        )
        ...
        if self.spec_aug and self.spec_aug_prev and self.model.training:
            spec_stacked = spec_augment(
                spec_stacked, mask_T=70, mask_F=4, num_T=2, num_F=2, p=1.0
            )  # (batch_size, seq_len, feature_dim * dr)
        spec_stacked = spec_stacked.to(
            device=self.device, dtype=torch.float32
        )  # (batch_size, seq_len, feature_dim * dr)
        pos_enc = (
            torch.FloatTensor(pos_enc)
            .to(device=self.device, dtype=torch.float32)
            .expand(spec_stacked.size(0), *pos_enc.size())
    ...
    def tile_representations(self, reps):
        ...
        tiled_reps = tiled_reps.reshape(
            reps.size(0), reps.size(1) * self.dr, reps.size(2)
        )
        return (
            tiled_reps  # (batch_size, sequence_length * downsample_rate, hidden_size)
        )
    ...
    def _forward(self, x):
        ...
        # Model forwarding
        spec_stacked, pos_enc, attn_mask = self.process_input_data(
            x
        )  # x shape: (B, T, D)
        x = self.model(
            spec_stacked,
            pos_enc,
            attn_mask,
            output_all_encoded_layers=self.weighted_sum or self.select_layer != -1,
        )  # (B, T, D) or # (N, B, T, D)
