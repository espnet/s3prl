...
def get_runner_args():
    ...
    parser = argparse.ArgumentParser(
        description="Argument Parser for the S3PLR project."
    )

    # setting
    parser.add_argument(
        "--config",
        default="../config/deprecated_runner/tera_libri_fmllrBase_pretrain,yaml",
        type=str,
    ...
    parser.add_argument(
        "--logdir",
        default="../log/log_transformer/",
        type=str,
        help="Logging path.",
        required=False,
    )
    parser.add_argument(
        "--name", default=None, type=str, help="Name for logging.", required=False
    )
    ...
    parser.add_argument(
        "--load",
        action="store_true",
        help="Load pre-trained model to restore training, no need to specify this during testing.",
    )
    parser.add_argument(
        "--ckpdir",
        default="../result/result_transformer/",
        type=str,
        help="path to store experiment result.",
        required=False,
    )
    parser.add_argument(
        "--ckpt",
        default="fmllrBase960-F-N-K-libri/states-1000000.ckpt",
        type=str,
    ...
    parser.add_argument(
        "--dckpt",
        default="baseline_sentiment_libri_sd1337/baseline_sentiment-500000.ckpt",
        type=str,
    ...
    parser.add_argument(
        "--run_transformer",
        action="store_true",
        help="train and test the downstream tasks using speech representations.",
    )
    parser.add_argument(
        "--run_apc",
        action="store_true",
    ...
    parser.add_argument(
        "--fine_tune",
        action="store_true",
        help="fine tune the transformer model with downstream task.",
    )
    parser.add_argument(
        "--plot",
        action="store_true",
    ...
    parser.add_argument(
        "--train_phone",
        action="store_true",
        help="Train the phone classifier on mel or speech representations.",
    )
    parser.add_argument(
        "--test_phone",
        action="store_true",
        help="Test mel or speech representations using the trained phone classifier.",
    )
    ...
    parser.add_argument(
        "--train_cpc_phone",
        action="store_true",
        help="Train the phone classifier on mel or speech representations with the alignments in CPC paper.",
    )
    parser.add_argument(
        "--test_cpc_phone",
        action="store_true",
        help="Test mel or speech representations using the trained phone classifier with the alignments in CPC paper.",
    )
    ...
    parser.add_argument(
        "--train_sentiment",
        action="store_true",
        help="Train the sentiment classifier on mel or speech representations.",
    )
    parser.add_argument(
        "--test_sentiment",
        action="store_true",
        help="Test mel or speech representations using the trained sentiment classifier.",
    )
    ...
    parser.add_argument(
        "--train_speaker",
        action="store_true",
        help="Train the speaker classifier on mel or speech representations.",
    )
    parser.add_argument(
        "--test_speaker",
        action="store_true",
        help="Test mel or speech representations using the trained speaker classifier.",
    )
    ...
    parser.add_argument(
        "--with_head",
        action="store_true",
        help="inference with the spectrogram head, the model outputs spectrogram.",
    )
    parser.add_argument("--plot_attention", action="store_true", help="plot attention")
    parser.add_argument(
        "--load_ws",
        default="result/result_transformer_sentiment/10111754-10170300-weight_sum/best_val.ckpt",
        help="load weighted-sum weights from trained downstream model",
    )
    parser.add_argument("--cpu", action="store_true", help="Disable GPU training.")
    parser.add_argument(
        "--multi_gpu", action="store_true", help="Enable Multi-GPU training."
    )
    parser.add_argument("--no_msg", action="store_true", help="Hide all messages.")
    parser.add_argument(
        "--test_reconstruct", action="store_true", help="Test reconstruction capability"
    )
